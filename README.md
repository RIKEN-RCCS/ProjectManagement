...
| `pm_minutes_import.py` | 文字起こし議事録をLLMで解析して `data/minutes/{kind}.db` に詳細保存（担当者・期限・出典付き）。`--delete` で削除 |
| `pm_minutes_to_pm.py` | `data/minutes/{kind}.db` の内容をLLM不使用でpm.dbに転記。`--delete` で削除 |
| `pm_meeting_import.py` | 文字起こし議事録をLLMで解析してpm.dbに直接保存（手動インポート用） |
...
#### 1.1. Slack APIを使いChannelの投稿を取得し、要約をChannelごとのDBへ登録
...
#### b. 議事録ファイル(Markdown形式)を手動でインポート
