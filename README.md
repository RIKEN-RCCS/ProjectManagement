...
```sh
bash scripts/slack_to_pm.sh -c <CHANNEL_ID>
```
...
```sh
source ~/.secrets/slack_tokens.sh
python3 scripts/slack_pipeline.py -c <CHANNEL_ID> --canvas-id <CANVAS_ID>

# 全体要約のみ再生成（Slack APIの取得・スレッド再要約をスキップ）
python3 scripts/slack_pipeline.py -c <CHANNEL_ID> --skip-fetch --canvas-id <CANVAS_ID>

# --since で期間を絞ってファイルにも保存
python3 scripts/slack_pipeline.py -c <CHANNEL_ID> --skip-fetch \
    --since 2026-03-01 --canvas-id <CANVAS_ID> --output overall.md
```
...
# 抽出済みスレッドの一覧確認
python3 scripts/pm_extractor.py -c <CHANNEL_ID> --list
```
...
bash auto_recording_import.sh -c <CHANNEL_ID>
```
...
python3 scripts/pm_minutes_import.py \
    --post-to-slack --meeting-name Leader_Meeting --held-at 2026-03-16 -c <CHANNEL_ID>

# 特定スレッドにリプライとして投稿（スレッドに集約、Files タブには表示されない）
python3 scripts/pm_minutes_import.py \
    --post-to-slack --meeting-name Leader_Meeting --held-at 2026-03-16 \
    -c <CHANNEL_ID> --thread-ts 1773791072.043109
```
...
```sh
source ~/.secrets/slack_tokens.sh
python3 scripts/pm_report.py --canvas-id <CANVAS_ID>

# 確認済み決定事項も含めて表示する場合
python3 scripts/pm_report.py --canvas-id <CANVAS_ID> --show-acknowledged
```
...
```sh
source ~/.secrets/slack_tokens.sh
python3 scripts/pm_sync_canvas.py --canvas-id <CANVAS_ID>
```
