...
def prepare_slides(
    ...
    *,
    ...
    """PPTX/PDF をスライドリストに展開する (画像化 + テキスト抽出)。

    Slide.image_png は work_dir 配下のファイルを指すため、後段の処理が
    終わるまで work_dir を残しておく必要がある。
    """
    src = src.resolve()
    if not src.is_file():
    ...
    slides = collect_slides(src, work_dir)
...
def summarize_all_slides(
    ...
    *,
    ...
    """各スライドを LLM で要約し、narration 文のリストを返す (画像 OCR 込み)。"""
    narrations: list[str] = []
    for slide in pm_tts._iter_progress(slides, total=len(slides), desc="summarize", quiet=quiet):
...
def render_video(
    ...
    *,
    ...
    """narration 列を TTS で WAV 化し、画像と mux して mp4 を作る。

    呼び出し側は prepare_slides で使った work_dir をそのまま渡す想定
    (slides.image_png が work_dir 配下を指しているため)。
    """
    if len(slides) != len(narrations):
        raise ValueError(
            f"slides と narrations の長さが一致しません: {len(slides)} vs {len(narrations)}"
        )
    ...
    for slide, narration in pm_tts._iter_progress(
        ...
        _synth_narration_to_wav(
            narration, wav_path, speaker=speaker, speed=speed, reference_id=ref_id,
        )
        pairs.append((slide.image_png, wav_path))

    _mux_video(pairs, output_mp4, work_dir)
    return output_mp4
...
def build_slide_video(
    ...
    *,
    ...
    """PPTX/PDF → mp4 を生成して output_mp4 のパスを返す (一気通貫の互換 API)。"""
    work_root = Path(tempfile.mkdtemp(prefix="slide_video_"))
    try:
        slides = prepare_slides(src, work_root, max_slides=max_slides)
        logger.info(f"スライド {len(slides)} 枚を処理")
        narrations = summarize_all_slides(
            slides,
            max_sentences=max_sentences,
            max_chars=max_chars,
            lang=lang,
        ...
        return render_video(
            slides, narrations, output_mp4, work_root,
            speaker=speaker, speed=speed, lang=lang, quiet=quiet,
        )
    finally:
