#!/usr/bin/env python3
"""RiVault / ローカル vLLM モデル評価スクリプト — Argus バックエンド適性テスト.

RiVault またはローカル vLLM で動くモデルが Argus バックエンドとして使えるかを多角的に評価する。
...
使い方:
  ...
  # ローカル vLLM ($OPENAI_API_BASE または --local-url) も評価対象に加える
  ~/.venv_aarch64/bin/python3 scripts/utils/eval_rivault_models.py --with-local
  ~/.venv_aarch64/bin/python3 scripts/utils/eval_rivault_models.py --local-models google/gemma-3-27b-it
  ~/.venv_aarch64/bin/python3 scripts/utils/eval_rivault_models.py --local-url http://localhost:9000/v1 --with-local
...
def fetch_models_from(base_url: str, token: str = "dummy") -> list[str]:
...
def _run_one(model: str, task: dict, *,
             url: str | None = None, token: str | None = None,
             is_local: bool = False) -> dict:
    """モデルにタスクを投げてメトリクスを返す。"""
    effective_url = (url or _RIVAULT_URL).rstrip("/")
    effective_token = token or _RIVAULT_TOKEN
    kimi = _is_kimi(model)
    ...
    if not kimi:
        # thinking: disabled はRiVault/Anthropic固有パラメータ。ローカルvLLMには送らない
        if not is_local:
...
def main() -> int:
    global _RIVAULT_URL, _RIVAULT_TOKEN, _LOCAL_URL, _LOCAL_TOKEN
    import os

    parser = argparse.ArgumentParser(
        description="RiVault / ローカル vLLM モデル Argus 適性評価",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    parser.add_argument(
        "--models", nargs="+", metavar="MODEL",
        help="RiVault から評価するモデルを指定（省略時: /v1/models から全取得）",
    )
    parser.add_argument(
        "--with-local", action="store_true",
        help="ローカル vLLM エンドポイントのモデルも自動検出して評価対象に追加",
    )
    parser.add_argument(
        "--local-url", metavar="URL",
        help="ローカル vLLM ベース URL（デフォルト: $OPENAI_API_BASE。未設定時は空）",
    )
    parser.add_argument(
        "--local-models", nargs="+", metavar="MODEL",
        help="ローカルエンドポイントで評価するモデルを明示指定（--with-local 不要）",
    )
    parser.add_argument(
    ...
    from llm import load_llm_secrets
    load_llm_secrets()
    ...
    _LOCAL_TOKEN = os.environ.get("OPENAI_API_KEY", "dummy")

    # --- モデルスペック収集: {"model": str, "url": str, "token": str, "is_local": bool} ---
    # spec の display_name はサマリーキーとして model を使うが出力では [local] prefix を付ける
    model_specs: list[dict] = []

    # RiVault モデル
    need_rivault = args.models or (not args.local_models)
    if need_rivault:
        if not _RIVAULT_URL:
            if args.local_models or args.with_local:
                pass  # ローカルのみなので RiVault 必須でない
            else:
        ...
        if _RIVAULT_URL:
            if args.models:
                rv_models = args.models
            else:
                try:
                    rv_models = fetch_models_from(_RIVAULT_URL, _RIVAULT_TOKEN)
                except Exception as exc:
                    print(f"[ERROR] RiVault モデル一覧取得失敗: {exc}", file=sys.stderr)
                    rv_models = []
            for m in rv_models:
                model_specs.append({"model": m, "url": _RIVAULT_URL,
                                    "token": _RIVAULT_TOKEN, "is_local": False})

    # ローカルモデル
    if args.local_models:
        for m in args.local_models:
            model_specs.append({"model": m, "url": _LOCAL_URL,
                                "token": _LOCAL_TOKEN, "is_local": True})
    elif args.with_local:
        try:
            local_discovered = fetch_models_from(_LOCAL_URL, _LOCAL_TOKEN)
            for m in local_discovered:
                model_specs.append({"model": m, "url": _LOCAL_URL,
                                    "token": _LOCAL_TOKEN, "is_local": True})
            print(f"ローカル ({_LOCAL_URL}): {len(local_discovered)} モデル検出")
        except Exception as exc:
            print(f"[WARN] ローカルモデル一覧取得失敗: {exc}", file=sys.stderr)

    if not model_specs:
        print("[ERROR] 評価するモデルがありません", file=sys.stderr)
        return 1

    # スキップ処理
    if args.skip:
        orig = model_specs
        model_specs = [s for s in model_specs
                       if not any(p.lower() in s["model"].lower() for p in args.skip)]
        skipped = [s["model"] for s in orig if s not in model_specs]
        if skipped:
    ...
    print(f"ローカル: {_LOCAL_URL}")
    print(f"評価モデル数: {len(model_specs)} / タスク: {', '.join(task_ids)}")
    ...
    for spec in model_specs:
        ...
        label = f"[local] {model}" if is_local else model
        ...
        for task_id in task_ids:
            ...
            metrics = _run_one(model, task,
                               url=spec["url"], token=spec["token"],
                               is_local=is_local)
            print(" 完了")
            score = _score(metrics, task)
            _print_result(label, task_id, task, metrics, score)
