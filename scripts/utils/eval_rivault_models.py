...
RiVault または localhost:8000 で動くモデルが Argus バックエンドとして使えるかを多角的に評価する。
...
使い方:
  ...
  # ローカル vLLM (localhost:8000) も評価対象に加える
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="ローカル vLLM ベース URL（デフォルト: $OPENAI_API_BASE または http://localhost:8000/v1）",
    )
    parser.add_argument(
