...
- **Classification gate (use when unsure whether an item is a decision)**: it IS a decision if at least one is true —
  (1) reversing it would force other work to be redone, (2) it rules out alternative options,
  (3) it commits resources (budget/people/compute) or direction (tech choice/schedule).
- **Capture context while it is still fresh (append as bracket tags after the decision, ONLY if stated in the transcript — do NOT infer or guess)**:
  `[根拠: なぜこの選択をしたか]` `[捨てた案: 検討したが採用しなかった代替案]` `[覆す条件: 何が起きたら見直すか]`
  Omit a tag entirely if the transcript does not state it — do not write "不明" or "なし" inside a tag.
...
Output format (EXACTLY 3 columns for アクションアイテム — do NOT add extra columns):
## 決定事項

- （決定事項）[根拠: ...] [捨てた案: ...] [覆す条件: ...]

（例: 上記3タグは transcript に明示されている場合のみ付ける。無ければタグ自体を省略する）
...
- **Bracket tags** (`[根拠: ...]` `[捨てた案: ...]` `[覆す条件: ...]`) may appear after a decision in the drafts.
  When merging duplicate decisions, keep the most complete tag values seen across the merged drafts
  (do not invent a tag that no draft stated; do not drop a tag stated by any merged draft).
