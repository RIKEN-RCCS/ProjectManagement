...
【修正対象の典型的な誤認識パターン】
- カタカナ転写: 英語アプリ名・ツール名がカタカナで音写されている場合は正式な英字綴りに修正する（例: 「スパック」→ Spack, 「ランブル」→ Ramble, 「オープンオンデマンド」→ OpenOnDemand）
- 略語の部分転写: アルファベット略語が部分的にしかカタカナで聞き取られていない場合は、用語辞書と整合する完全な略語表記に修正する
