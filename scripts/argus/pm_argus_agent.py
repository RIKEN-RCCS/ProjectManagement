...
- [ ] 「もし要件 X を採用したら何が起きるか」の **技術的影響・実装制約** が書かれているか（例: 規格外になる、特殊設計が必要、他コンポーネントへの波及）
- [ ] **閾値・境界条件**（"X 以下なら標準対応、超えると特殊"）が書かれているか
- [ ] **代替案や縮退オプション** とその影響が書かれているか
- [ ] **賛成側と反対側の両方** の意見・懸念が書かれているか
- [ ] **数値・日付・人名・会議名** で根拠が裏付けられているか

上記の 3 項目以上が抜けている場合は、追加のツール呼び出しで補完してから回答すること。
