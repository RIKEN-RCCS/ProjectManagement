...
- 複数のアプリ・エンティティ・論点を跨ぐ質問では、まとめて1クエリにせず、
  エンティティごとに個別の検索クエリに分解して並列実行すること
  （一括クエリでは各エンティティの取り分が上位件数に食われて失われる）
...
- 本日: {today}
- 対象期間: {since} 〜 {today}
- Slack・議事録・Web記事は上記期間で絞られる。BOX文書（報告書等の正式文書）は
  期間外でも検索対象になる（文書の日付は「情報の発生日」ではないため）
- 期間を変えたい場合は search_text / search_text_hybrid の `since` 引数を使う
  （YYYY-MM-DD 指定、`"all"` で無期限）
"""
