...
def detect_knowledge_conflicts(ctx) -> int:
    """knowledge.db.knowledge_relations の `conflicts_with` 新規行を検出して通知する。
    プロジェクト全体のナレッジ整合性を保つ仕組み。
    """
    from .actions import send_channel_alert
    from db_utils import open_knowledge_db

    cfg = ctx.config.get("patrol", {}).get("knowledge_conflict", {})
    if not cfg.get("enabled", True):
        return 0
    ...
    try:
        kdb = open_knowledge_db(db_path, no_encrypt=False)
    except Exception as e:
        logger.warning("knowledge.db 接続失敗: %s", e)
        return 0

    try:
        # lookback_days 以内に作成された conflicts_with 行を取得
        from datetime import datetime, timedelta, timezone
        cutoff = (datetime.now(timezone.utc) - timedelta(days=lookback_days)).isoformat()
        rows = kdb.execute(
            "SELECT r.from_id, r.to_id, r.note, r.created_at,"
            "        kf.topic AS from_topic, kt.topic AS to_topic"
            " FROM knowledge_relations r"
            " LEFT JOIN knowledge kf ON kf.id = r.from_id"
            " LEFT JOIN knowledge kt ON kt.id = r.to_id"
            " WHERE r.relation = 'conflicts_with'"
            "   AND r.created_at >= ?"
            "   AND COALESCE(kf.deleted, 0) = 0 AND COALESCE(kt.deleted, 0) = 0"
    ...
    for r in rows:
        ...
        if ctx.state.already_notified("knowledge_conflict", target_key, cooldown):
            continue
        if sent >= max_per_run:
            logger.info("knowledge_conflict: max_per_run=%d に達したため打ち切り", max_per_run)
            break

        from_topic = (r["from_topic"] or "(不明)")[:60]
        to_topic = (r["to_topic"] or "(不明)")[:60]
        note = (r["note"] or "").strip()
        text = (
            f":warning: *ナレッジ間で矛盾が検出されました*\n"
            f"・*{r['from_id']}*: {from_topic}\n"
            f"・*{r['to_id']}*: {to_topic}\n"
            + (f"\n根拠: {note}\n" if note else "\n")
            + "確認・整理:\n"
            f"  • `/argus-knowledge {r['from_id']}` または `/argus-knowledge {r['to_id']}` で詳細\n"
            "  • どちらかを `/argus-knowledge invalidate KN-XXXX` で無効化、もしくは"
            f" `/argus-knowledge supersede KN-XXXX KN-YYYY` で上書き連鎖を作成してください。"
        )

        if send_channel_alert(ctx, leader_ch, text) and not ctx.dry_run:
            ctx.state.record_notification(
                "knowledge_conflict", target_key, leader_ch
            )
        sent += 1

    if sent:
        logger.info("ナレッジ矛盾通知: %d 件送信", sent)
    return sent
