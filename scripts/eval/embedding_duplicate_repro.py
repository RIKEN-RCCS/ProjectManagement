...
# このスクリプトが行う 4 つのテスト（各テストは PASS/FAIL を出力する）
  ...
  2. truncation   : 長文 A と A の先頭 k 文字を埋め込み、embed(A) == embed(A[:k])
                     ...
                     切り詰められている」ことの証明。モデル最大入力長による
                     仕様の可能性が高いため WARN 扱い（総合結果には影響しない。
                     衝突バグとは別事象として参考報告する）。
...
# 期待される正常挙動 / 異常挙動
    ...
    - truncation : 「一致する k なし」が最もクリーンだが、大きい k
                    ...
                    仕様として起こり得る（WARN 表示・総合結果に影響しない）。
                    ごく小さい k で一致する場合のみ異常を疑う。
...
def test_truncation(client: EmbedClient, ks: list[int]) -> bool:
    ...
    if matched_ks:
        ...
        print(f"  [{_WARN}] embed(A) と一致する最小の k = {min_k}"
              f"（入力がおよそ {min_k} 文字相当で切り詰められている可能性。"
              "モデル最大入力長による仕様の可能性が高く、衝突バグとは別事象。"
              "総合結果には影響させない）")
        return True
...
def test_pairs_file(client: EmbedClient, pairs_file: str, preview_chars: int) -> bool:
    ...
    for i, pair in enumerate(pairs):
        ...
        if not ok:
            all_ok = False
    if collisions:
        print(f"\n  ★ 異常の実証: 内容の異なるテキスト同士がバイト完全一致のベクトルを返したペア = {collisions} 件")
        print("    （PASS はスクリプトの期待どおりという意味であり、エンドポイントの挙動としては異常です）")
    return all_ok
