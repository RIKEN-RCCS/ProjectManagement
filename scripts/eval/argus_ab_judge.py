...
EVAL_DB = Path(os.environ.get("AB_DB") or (REPO / "data" / "eval" / "v4flash_ab.db"))
...
def cmd_run(args):
    ...
    for sid, kind, _title, sysp, userp in rows:
        # 指定ペアの trial を取得
        ta = con.execute(
        ...
        prompt = (
            f"# 対象プロンプト\n## system\n{sysp}\n\n## user\n{userp[:6000]}\n\n"
            f"# 出力 A\n{oa[:args.max_out_chars]}\n\n# 出力 B\n{ob[:args.max_out_chars]}\n\n"
...
def main() -> int:
    ...
    r.add_argument("--max-out-chars", type=int, default=4000,
                   help="judge に見せる出力 A/B の最大文字数（brief/risk の長文出力の"
                        "短文有利バイアス対策。既定4000は後方互換）")
