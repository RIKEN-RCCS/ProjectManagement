...
## カテゴリ
- completed: プロジェクト推進に関わる**重要な完了マイルストーン**を最大5件。
  **性能評価の実績（性能測定の実施・結果、対富岳比・GPU世代間比較、スケーラビリティ、実機評価、
  ベンチマーク測定）を最優先**し、レポートに存在する限り枠の過半をこの種別で埋めること。
  GPU移植・OSS公開・EEA登録などの作成・登録系は枠が余る場合に代表的な1〜2件のみ。
  レポートが実績として記述している事項（〜済み / 〜公開 / 〜実施 / 測定完了 等）は、明示的な
  完了報告メッセージが無くても completed に含めてよい。ただしレポートに記載の無い推測は書かない。
  各項目は名詞止めの短い一句（20字以内）。日付があれば末尾に (2025-12) の形で添える。
  資料作成・会議記録・「〜に言及」「対応を記録」等の手続き的メモは書かず、実質的な成果のみ。
