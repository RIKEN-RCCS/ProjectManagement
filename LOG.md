...
## 2026-08-03 Kimi-K3 を「書き手」ではなく「読み手」として入れた

**背景**: セキュリティ対策が一巡したので K3 の議事録適用を再検討した（PM 起案）。
5 会議 × 4 アームを**同一時期に**測り直した（前回は 7/31 と 8/03 のデータが混在しており、
異なる時期の A と C を比べていた。RIKYU の配信モデルが変わっていないことは確認できないため
その勝敗表は使えなかった — R12）。
**決定**: **画像入力は中止。K3 は読み手（欠落検出）に限定して恒常運用へ。**
**判断の根拠**: ①用語一致率が glm の 4 割（46.5%→18.7%）で ASR 誤認識の復元ができていない
②所要 3.5〜4.2 倍 ③実測 3 会議で検出した 19 件の欠落候補を PM が読み「載っているべきだった」と
確認した（日付を伴う約束が 6 件、マイルストーン期限に関わるものが 2 件）。
**書かせず読ませるなら弱点が問題にならない** — 形式を要求せず、クリティカルパス外で、
失敗しても議事録は完成済みで、出力は pm.db の action_items/decisions に入らない。
**judge が再現しないことが分かった**（同じ出力を再判定して C:B が 0%→66.7%、D:A が 60%→80% に
反転）。**この A/B 設計では K3 の優劣を判定できない**ので、判断は定量メトリクスと人の確認に寄せた。
**500/504 は時期依存でモデルの性質ではない**（7/31 は K3 系 6/6、8/03 は 1/9）。
504 は無通信タイムアウトでもない（1215s で成功する一方 945s で 504。**既にストリーミング済み**）。
**K3 は R8 の独立系統ではない**（Moonshot は Zhipu と同系統）。Llama-4-Scout の代わりにせず
`--reader both` で両方を回す。**pin は役割を区別しない**ので、生成に使わないのは運用規律による。
...
## 2026-08-02 Patrol の Read Plane 分割を見送り — 監査記録の欠如を先に埋めた

**背景**: 能力分離 5b の対象を Patrol にも広げるか検討した。Patrol の LLM はツールを持たず
宛先も選べない（送信先は config 由来、判定は verdict 文字列を返すだけ）ため、
「読取と送信の同居」の危険が investigate と違ってモデルからは行使できない。分割には
親をブローカーにする IPC（読取と送信が交互に走り、送信の戻り値を承認フローが使う構造）が
必要でコストに見合わない。
**決定**: 分割は見送り、代わりに実在した穴（LLM 判定が本番データを書き換えるのに記録が無い）を
`patrol/audit.py` で埋め、3 つの LLM 判定と自動クローズを `tool_calls` / `reasoning_traces` に
記録するようにした。
**影響**: Patrol は cron から意図的に外れたまま（DM の信頼性の問題が別途未解決のため）。
本番実行での監査記録の確認は自動クローズが走ってしまうため保留。
...
**背景**: 外部からの指摘に始まる対策で流出面（層1・層2）は enforce まで到達したが、**検知は
