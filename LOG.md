...
## 2026-08-05 noise 判定 1,661 件の 9 割は「実行単位の事故」だった — モデルの判断力の問題ではない

**当初の見立ては誤りだった**。noise 判定の理由文の 79% が core を正当化していたため
「`judge_batch` がラベルを取り違えている」と疑ったが、**再現テストで否定された**。
同じ 5 件をバッチ5件・1件ずつの両方で判定させたところ、生の LLM 応答は
`{"relevance": "core", ...}` を正しく返し、パースも正常だった。**LLM は正しく、DB が誤っている。**
**判定日時でグループ化して原因が見えた**: 2026-05-15 19:09（842件）/ 2026-07-21 09:56（614件）/
2026-05-15 19:42（44件）の 3 実行が **100% noise・core 0 件**。noise 全体 1,661 件の 9 割がここ。
健全な実行（2026-05-15 12:07、328件）は判定が混在し、noise の理由も本当に noise を
正当化している（「章立てのみ」「カバーレター」）。事故った実行の理由は全件が core を指す。
**範囲の判断ではない**: 取得元・フォルダの構成比は健全な実行とほぼ同じで、全体に散らばって
いる。「この領域は索引対象外」という人の判断なら偏るはずで、偏っていない。
**cron 経由でもない**: `pm_box_update.log` に残る `--judge` の判定対象は最大 87 件で、
842/614 件の実行は記録に無い（手動実行）。
**機構**: `cmd_import` は `relevance` を更新するが `relevance_reason` は書き換えない。
だから「noise ラベル + 以前の判定が書いた core の理由」が残る — 症状と一致する。
退化した `--judge` 実行の線も残るが、1,456 回連続でモデルが自分の理由と逆のラベルを
選んだと考えるより、CSV 経路の事故と見る方が自然（Excel の列ズレ・フィルダウン）。
**対処**: `--rejudge-relevance noise` を追加（現在その判定値の行だけ再判定。健全な 328 件を
巻き込まない）。実行前に全 2,041 件を CSV へ退避し、`--import` で戻せるようにした。
**教訓**: 「LLM の判断が悪い」で止めると、モデル差し替えという高価で無効な対策に進む
ところだった。**理由文とラベルの不整合という観測から、実行単位の事故に辿り着いた。**
データ品質の異常は、まず「いつ・どの実行で入ったか」で切ること。

## 2026-08-05 Box relevance の noise 判定を掘り返す経路を作った — 主系統モデルの差し替えより先に測る

**背景**: Box は「LLM が読む量が最も多いのに人手のレビュー段が最も薄い」入力である。
2,041 件中 1,661 件（81%）を noise で捨てて索引から外しているのに、**その 81% が正しい
という根拠が無かった**。主系統を GLM-5.2 から Kimi-K3 に替える案が出たが、(a) K3 は
主系統と同じ供給元クラスタなので独立性は増えない (b) 議事録の読み手が既に K3 なので
同じモデルが「何を索引に残すか」と「議事録に何が欠けているか」の両方を決めることになり、
現状ある「違う間違い方をする」性質を失う (c) 第2系統向けの `max_tokens=256` を流用すると
K3 は thinking で使い切って**全部 unknown に落ちる**（しかも unknown は正常値に見える）。
**測る前にモデルを替えるのは根拠が無い**と判断し、差し替えは見送った。
**決定**: `pm_box_relevance.py` に `--recheck-noise --reader k3` を足した。既存の noise
判定を読み手に再審査させ、**relevance は上書きせず** `triage_second_opinion` に記録する。
`--judge` 同梱の差分検査は「今まさに判定した行」しか見ないため、既に確定した山には
永久に届かなかった（これが掘り返しの入口が要る理由）。
**設計の非対称**: R8 の第2系統はフラグ語で対象を絞るが、recall チェックは絞らない。
狙いが違う — R8 は「特定の話題が狙って落とされていないか」、recall は「読み落としで
落ちていないか」。後者に話題の絞り込みを入れると大多数が検査対象から外れる。
一致・不一致の**両方**を記録する（不一致だけでは率として読めない）。
**初回実測**: 6 件検査して **6 件すべてが noise を覆した**（core 5 / related 1）。
WG 資料・キックオフ議事メモまで落ちており、noise 判定は過剰な疑いが濃い。
**保留との関係**: `--reader k3` は R8 の hold を迂回する。**これは R8 対策ではない**と
ログにも記録にも明示した（kind を `box_relevance_recall` に分け、R8 の `box_relevance` と
別集計にする）。K3 で「対策済み」に見せないための分離。

## 2026-08-05 Box relevance の編集画面を Console に足した — 修正経路が CSV だけでは使われない

**背景**: relevance の修正経路は `--export` → CSV 編集 → `--import` しか無く、2,041 件を
人が見る道具として実用に耐えなかった。**修正できない検査は、実行しても索引が 1 件も
変わらない** — K3 の再審査を回しても人が CSV を往復しなければ何も起きず、
「実装したが効いていない」の一例になるところだった。
**決定**: Actions/Decisions と同じ ag-Grid のタブ（`#boxdocs`）を追加。既定で noise を
先に見せ、複数行を一括で core/related に戻せる。読み手(K3) の再審査結果を突き合わせ、
「主系統 noise × 読み手 core」の行を強調する（見るべき順序を画面が持つ）。
**値が変わった行だけ書く**: 全行 UPDATE にすると、画面を開いて何も触らず保存しただけで
全件が `relevance_source='human'` になり、LLM の再判定対象から永久に外れてしまう。
**本文字数を列に出した**: relevance の誤りと本文抽出の失敗を見分けるため。
極端に短い行は「情報不足」で noise に倒れている疑いを持てる。

## 2026-08-05 relevance に人手修正の保護列を入れた — 人間の最終判断を LLM が黙って上書きしない

**背景**: `box_files` には「人間が直したか LLM が判定したか」を区別する列が無く、
`--judge --force` を打つと CSV 経由の人手修正が黙って消えていた。議事録側には
human_kept の保護（`ingest/minutes.py`）を入れてあったのに、Box には無かった。
**決定**: `relevance_source`（`human` / `llm` / NULL）を後付けし、`--force` でも
`human` 行は再判定しない。覆すには `--force-human` の明示が要る。
**NULL を `llm` に丸めなかった**: 既存 2,041 行は由来不明で、CSV 経由の人手修正が
混じっている可能性がある。「分からない」を「LLM が付けた」に丸めると、保護すべき行を
保護しないまま「保護済み」と表示することになる。由来不明は保護対象外だと `--stats` で明示する。

## 2026-08-05 brief/risk の Orchestrator-Worker 記述を docs から消した

**背景**: `docs/architecture.md` は brief/risk を「Worker A/B/C + Orchestrator」と書いていたが、
2026-07-23 の盲検 A/B 以降そのコードは存在せず、全文脈 single-shot に置き換わっていた。
**決定**: 図と節を実装に合わせ、**investigate 系が唯一の agent ループ**であることを明記した。
これは単なる記述の正確さではなく、**LLM が自分で行動を選ぶ箇所が `run_agent()` 1 箇所に
閉じている**という前提が、allow-list・plane 制限・`tool_calls` 台帳の置き場所の根拠だから
（図が古いと「brief にも Worker が居る」前提で対策範囲を誤る）。

## 2026-08-04 R8 の第2系統を保留した — 能力の足りないモデルで「対策済み」にしない

**背景**: Scout（Llama-4-Scout-17B）は役割に耐えなかった。同一会議・同一 VTT の実測で
①応答形式を守らない（この逸脱で議事録経路は一度も所見を記録できていなかった）
②ASR 誤認識を復元できない（K3 が NVIDIA / cuFFT / KDDI と読んだ箇所を誤認識のまま出す）
③記録された 10 件すべてが3ゲート審査で DROP。**置いてあるが実質的に何も言っていない。**
**PM 判断**: GLM-5.2 / Kimi-K3 に相当する能力で、かつ出自が主系統の外にあるモデルが
現時点で無い。**能力の足りないモデルを置いて「対策済み」にするより、無いと認めて保留する。**
**決定**: `config/sensitive_terms.yaml` の `second_opinion.on_hold` を単一の宣言元にし、
第2系統を呼ぶ 3 経路（議事録欠落検査の `--reader second` / Slack Pass 1 抽出 /
Box relevance）すべてが**LLM を呼ばずに WARN を出して飛ばす**。チョークポイント
（`_call_second_opinion_extraction`）は**空の結果ではなく例外**にした — 「0 件の不一致」と
「そもそも検査していない」を混同させないため。
**帳簿を直した**: PLAN.md の完了リストから R8 を取り下げ、`docs/security-architecture.md`
の R8 行にも保留を書いた。**保留を「実装済み」の欄に残さない。**
**保留が忘れられるのを防ぐ**: `pm_selfcheck` の `second_opinion_on_hold` が毎回報告する
（観測であり違反ではないので exit code には影響しない）。今日「実装したが効いていない」
制御を 8 件見つけた後で、**保留を黙って放置すれば 9 件目になる**。
**テストの扱い**: 機構のテスト（突合・重複排除・3ゲート・上限）は autouse fixture で
保留を外して維持する — 保留を解いたときに壊れていては意味がない。保留の挙動は別クラスで固定。

## 2026-08-04 供給網の検査は「宣言→実在」しか見ていなかった — 逆向きを足した

**背景**: RiVault にモデルが 9 本増えた（Kimi-K3 / GLM-5.2 / GLM-OCR / RiVault 自製 6 本）。
**`model_pin.py --check` は全件 OK のままで、気づいたのは PM からの連絡だった。**
検査は宣言したモデルの実在しか見ておらず、**知らないモデルが増えても違反にならない**。
`net_guard` 層3 で `egress_dest_unknown`（知らない宛先を報告）を作ったのに、
モデル側に同じものを置いていなかった。
**決定**: `unknown_served()` / `--list-served` / `model_pin_unknown_served`（selfcheck）で
逆向き（実在→宣言）を見る。新しい id は `observed_not_used:` に人が追記して認める形にし、
**追記が git の diff に残る**ことを変更検知にする（`declared_*` と同じ作法）。
**観測であり違反ではない** — 配信内容は運用主体の判断であり Argus の不変条件の破れではない。
**もう一つ分かったこと（P10）**: `/v1/models` の応答は**トークンのスコープに依存する**。
同じ RiVault が旧トークンで 15 件、更新後で 24 件を返した。**旧トークンでの「OK・15件」は、
配信 24 件のうち 9 件が見えていない状態**だった（見えないものは照合もされない）。
pin が言えるのは「**このトークンで見える範囲では**宣言と一致する」までである。
**R8 への影響**: RiVault 側に Moonshot（K3）と Zhipu（GLM-5.2）が入り、**第2系統の置き場
だった RiVault にも主系統と同系列が増えた**。RIKYU 側の 4 本は元々すべて中国系。
一方で `K2-Think`（UAE）と **RiVault 自製モデル 6 本**が見えた — 後者は出自が運用主体自身で、
**R8 に対して最も独立した第2系統の候補**（Scout は ASR 誤認識を復元できず所見が全部
審査で落ちたため、差し替え先の実測が要る）。

## 2026-08-04 所見を件数ではなく内容で絞る — 3ゲート審査を記録段に入れた

**背景**: PM から「30 件は多すぎる。このままだと見ない。**件数で絞るのではなく本当に
プロジェクト推進に欠かせない内容だけに絞るべき**」。上限を上げたばかりだが、そもそも
件数は絞る軸ではなかった。
**決定**: 記録前に既存の3ゲート審査（`triage_items_batched`、プロンプトは複製しない）を
通し、`gate_verdict` / `gate_reason` 列に KEEP/DROP と理由を残す。既定の表示は KEEP のみ。
**DROP でも行は消さない**（表示から外すだけ）。**未審査（NULL）は隠さない** — 審査が
動いていないことに気づけなくなるため、KEEP と同じく表示する。
**実測**: 5 会議 69 件 → KEEP 28 / DROP 41。今日の会議は 40 → 13。決定（方針・体制・期日・
適用範囲）が残り、「〜を共有する」「〜を調整する」型が落ちた。
**PM 判断**: DROP の中に期日付きの依頼が含まれる（8/03 に PM が価値を認めた型の一部）。
「今のままで良い」— 期日付きの依頼はアクションアイテム側で管理する。
**残る弱点**: 審査は主系統のLLMが行う。**主系統が落とした項目を主系統自身に「重要でない」と
判定させている**構図で、R8 の独立性を部分的に損なう。行を残し DROP 件数を常に見えるように
してあるのは、その抑圧が起きたときに観測できるようにするため（DROP 比率の上昇を疑う）。
**踏んだ穴**: gate 列の後付けを「判定があるときだけ」にして `no column named gate_verdict`
で全テストが落ちた。**NULL を入れる場合も INSERT の列名に書くので後付けが必要。**

## 2026-08-04 読み手の抽出は再現しない — 上限を last resort に振り切り、重複排除を入れた

**背景**: 今日の Leader_Meeting を K3 に再度読ませたら、**同一会議・同一VTT・同一モデルで
検出数が 18 件 → 26 件に変わり、共通は 3 件だけ**だった。judge が再現しないことは 8/03 に
分かっていたが、**抽出そのものも再現しない**。
**決定1**: 記録上限を 10 → 25 → 100 にした。**上限に張り付いている限り「全部見た」と言えない**。
100 は妥当な件数ではなく**事故防止の last resort** — 突合が壊れて全件が「欠落」になったときに
台帳を埋め尽くすのを止めるためだけの値で、到達したら読み手か突合が壊れている兆候として読む。
量の制御は記録が残る側（レビュー・プロンプト・重複排除）でやる。**打ち切りは本文がどこにも
残らないので、後から「何を捨てたか」を検証できない**のが上げた理由。
**決定2**: 記録前に同じ会議・同じ読み手（kind）の既存所見と突合し重複を記録しない
（`_extraction_texts_match`。完全一致では表現の揺れを捕まえられない）。読み手が違えば
別に記録する — Scout(R8) と K3(recall) は別系統で、片方の所見を他方の重複として消せない。
**捨てた案**: 既存 10 件を削除してから上限を上げて再実行する案（PM に提示して撤回）。
**再実行しても同じ 18 件は出ない**ため、削除すると確認済みの所見を失う。実際 7 件が
2 回目には出なかった。**再現しないものを消して入れ直してはいけない。**
**副産物**: 同じ会議を複数回読ませると検出が積み増せる（重複排除が入ったので台帳が汚れない）。
重要な会議は複数回かけるのが有効という運用が導けた。

## 2026-08-04 第2系統（Scout）は議事録経路で一度も所見を出していなかった — 応答形式の逸脱で毎回落ちていた

**背景**: Console から議事録作成を起動したら第2系統検査が `AttributeError:
'str' object has no attribute 'get'` で失敗した（`logs/admin_job_25725851.log`）。
原因は Llama-4-Scout が `{"decisions": [{"content": ...}]}` ではなく
**`{"decisions": ["素の文字列", ...]}` を返す**こと。プロンプトで形式を指示しても従わない。
**決定**: 抽出結果を `extraction_item_contents` で正規化する（素の文字列・キー名違い・
配列でない文字列・top-level が dict でない場合を吸収し、**逸脱があれば必ず warn を出す** —
正規化で「モデルが指示に従っていない」事実を見えなくしない）。突合段の例外も
チャンク単位に閉じ込め、スキップ件数を要約に必ず出す（**0 件でないなら「全部見た」と言わない**）。
**分かったこと**: `triage_second_opinion` の `minutes_extraction`（Scout）は**この修正まで 0 行**だった。
8/03 の検証は `--reader k3` 単独で回しており、**`--reader both` が実データに当たったのは今回が初めて**。
`readers` の順で Scout が先に走るため、**Scout の例外で後続の K3 も走らず、インライン検査は何も出さなかった**。
**また同じ型**（「実装した」と「効いている」は別）。今回の型は
**「片方の読み手でしか検証していないのに両方を既定にした」** — 既定値は検証した構成と一致させる。
