...
6. **知識ベース（Vault）との分担** — `LOG.md` は「**このコードがなぜこうなっているか**」に限る。
   文献の検算・複数案件にまたがる原則・**誤りの型**は Vault 側に置き、`LOG.md` には書かない。
   **Vault の記述を `LOG.md` へ転記しない** — origin は public な GitHub なので、
   人名・アプリ名・Slack ID を含む記述が履歴に残る（削除しても公開履歴からは消えない）。
   参照は Vault → リポジトリの一方向にし、コミットハッシュで指す。
   **リポジトリ側から Vault のパスを書かない**（ディレクトリ構造が公開履歴に残るため）。
...
- `data/pm.db` — action_items / decisions / meetings / goals / milestones / achievements の唯一の正本。`decisions` は rationale 付き、`achievements` はアプリ別確定実績台帳
...
- **新規スクリプトの作成は最小限に**。変更は既存のスクリプトを拡張・修正して対応することを最優先とする。新規スクリプトを作成するとバグの混入箇所が増え、テストもレビューも必要になる。何かを達成したいときは、まず `scripts/` 配下に同様の処理を行う既存スクリプトがないか探し、それを再利用・修正する形を取ること。
...
## モデル運用ポリシー（Claude Code セッション）

役割ごとにモデルを使い分ける。**オーケストレーター自身のモデル・effort は本ファイルからは
設定できない**（ユーザーが `/model` で設定する。想定: Fable 5 / effort max）。

**オーケストレーター（メインセッション）の振る舞い**:
- 担当は計画・分解・委譲・統合・最終判断のみ。**自らコードを書かない** — 実装は必ず
  サブエージェントへ委譲する。例外: 数行の自明な修正、PLAN.md/LOG.md 等のドキュメント編集は直接行ってよい

**委譲マッピング**（`.claude/agents/` の定義で model 固定済み）:
- 実装・テスト・ボイラープレート・雑務 → `argus-implementer`（Sonnet）
- コードレビュー・コミット前点検 → `argus-code-reviewer`（Opus）
- 設計判断・複雑なデバッグ（該当エージェントが無い場合）→ Agent ツールで `model: "opus"` を明示
- DB調査 → `pm-db-analyst`（Sonnet）／運用・デーモン → `argus-ops`（Sonnet）／
  ドキュメント規律 → `docs-keeper`（Sonnet）

**Codex セカンドオピニオン**（別視点のシニアエンジニア）:
- 高リスクな判断（アーキテクチャ変更・後戻り困難な決定・本番データに触れる設計）では、
  Bash で `codex exec "<相談内容>"` を呼び第二意見を得てから判断する（CLI導入・認証済み）
- **機密ファイル（docs/project.md / argus_config.yaml / goals.yaml / ~/.secrets）の内容・実値を
  プロンプトに含めない**。渡すのはコード・差分・設計の記述のみ
- これは開発時のツール利用であり、本システムのスクリプト実行時のLLM制約（rivault/local のみ）とは別枠
...
- `pm-argus-commands` — `docs/argus_outcomes.md`。Argus 8 コマンド（brief/today/draft/risk/direction/investigate/transcribe/narrate）の使い方・引数・内部ツール仕様を触るとき
