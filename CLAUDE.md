...
| チャンネルID | 用途 | 対応Canvas |
|---|---|---|
| `<CHANNEL_ID>` | 20_1_リーダ会議メンバ | `<CANVAS_ID>` |
| `<CHANNEL_ID>` | personal | `<CANVAS_ID>` |
...
[Slack] ─── slack_pipeline.py ───→ {channel_id}.db
                                          ↓
[会議議事録] ── meeting_parser.py ──→ pm.db ←─ pm_extractor.py
  meetings/*.md                    (決定事項・               ↑
...
│   ├── meeting_parser.py            # 議事録 → pm.db（1ファイル単位）
│   ├── pm_bulk_import.py            # meetings/ の議事録を一括で pm.db に登録
...
└── data/                            # DBと出力ファイル
    ├── {channel_id}.db              # Slackデータ（例: <CHANNEL_ID>.db）
    ├── pm.db                        # PM統合データ
...
| `--canvas-id ID` | `<CANVAS_ID>` | 投稿先CanvasID |
...
### 3. 会議議事録の一括登録（pm_bulk_import.py）

`meetings/` ディレクトリ内の `YYYY-MM-DD_{会議名}.md` ファイルを一括で pm.db に登録する。ファイル名から `--held-at` と `--meeting-name` を自動抽出して `meeting_parser.py` を順次呼び出す。`_parsed.md` で終わるファイルは対象外。
...
### 3a. 会議議事録 → pm.db（meeting_parser.py、1ファイル単位）
...
```sh
# 通常運用: 未処理スレッドのみ抽出
python3 scripts/pm_extractor.py -c <CHANNEL_ID>

# 確認用（DB保存なし）
python3 scripts/pm_extractor.py -c <CHANNEL_ID> --dry-run --output result.txt
```
...
python3 scripts/db_migrate.py data/pm.db data/<CHANNEL_ID>.db data/<CHANNEL_ID>.db
...
- `meetings/*.md` をLLMで解析し `pm.db` に構造化保存（`meeting_parser.py`）
