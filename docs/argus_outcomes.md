...
ファイル名の一部を指定すると、`box_docs.db` でファイル名から `box_file_id` を解決し、
該当資料（最大 **3 ファイル・合計 400,000 字**まで）の **本文全文**（図言語化込み）を
**24,000 字窓（1,000 字オーバーラップ、ページ境界 `\f` を優先して分割。窓サイズは
env `ARGUS_DOC_QA_WINDOW` で上書き可、A/B 測定用）** で読み込み、
断片ごとに関連事実を抽出（map）→ 最後に全断片の抽出結果から統合回答を生成（reduce）する
専用パス（`run_document_qa`）。検索エージェント（ツール選択・`search_text` 系・FTS5/embedding
検索）は一切経由しない ——1 文書スコープの質問は「検索」ではなく「全文読込」が正しい
プリミティブという判断による。`--file` なしの通常経路は従来どおり検索エージェント
（`run_agent`、下記ツール一覧）を使う。

ファイル名が0件解決の場合は調査を開始せず**エラー停止**する（変更なし）。出力先フラグ
（`--to-box` 等）と併用可（変更なし）。

処理限界に該当した場合は回答末尾に **`## 制限事項`** セクションを機械的に付与して明示する
（誇張・言い繕いをしない）:

| 限界 | 表示条件 |
|---|---|
| ファイル数/合計文字数の上限超過（3ファイル・400,000字） | 上限超過分は「対象外」として除外ファイル名を明示 |
| タイムアウト予算切れ | 未読込の断片名を明示（既読分のみで回答を生成） |
| `doc_content` 本文未取得（未変換ファイル） | 「本文未取得（未変換）」として対象外ファイル名を明示 |
...
**`search_text` / `search_text_hybrid` の期間フィルタ免除**: 既定の期間フィルタ（`--days`/`--since`、
既定30日）は `source_type='box_document'`（Box正式文書）には適用されない。正式文書は鮮度に関わらず
常に検索対象になる。
