...
プロジェクトマネージャーが Slack から呼び出せる 6 つのコマンドの実践的な使い方まとめ。
...
| `/argus-direction`  | 意図した優先度と実際の決定の投入量のズレを確認したい | ephemeral | 10〜30秒 |
...
## 4. `/argus-direction` — 方向Δレポート（決定クラスタ集約）

「マイルストーンを全て達成しても、要件に合致しないものを作ってしまう」失敗を防ぐための
コマンド。Slack会話・議事録は見ず、前提・意思決定台帳（`ledger_edges`）だけを見て、
「意図した優先度」と「実際の決定の投入量」のズレ（Δ）を機械的に洗い出す。
`/argus-brief`/`/argus-risk`と異なり自由なLLM要約ではなく、グラフ構造による
集合化＋LLMは命名のみという制限された使い方をする（もっともらしい誤りを防ぐため）。
...
1. **投入不足領域** — 重要度は高いのに決定がほとんど無い目標
2. **未着手の目標** — 貢献する決定が1件も無い目標
3. **非収束の警告** — 同じ目標に対して方向性の異なる決定群が併存している場合
4. **決定クラスタ一覧** — 目標ごとに、貢献している決定をグルーピングして提示
   （各グループにLLMが短い名前を提案。名前はあくまで提案で人の確認が前提）

### 想定シーン

- 「マイルストーンは順調なのに、なぜか手応えが無い」ときに構造面から原因を探る
- 四半期の振り返りで「重視すると言っていた領域に実際どれだけ投入したか」を確認
- 複数のサブグループが実は矛盾する方針で動いていないかの点検

### 前提データ

決定事項が `ledger_goals`（目標）・`ledger_assumptions`（前提）に紐づいている必要がある
（`enrich_items.py` によるエンリッチメントで自動生成）。紐づけがまだ無い場合は
「集約対象がありません」とだけ表示される。

---

## 5. `/argus-investigate` — マルチステップ調査（Agent）
...
このギャップを埋めるため、LLM が元クエリから「本文に出てきそうな別表現」を 2 パターン生成し、**計 3 クエリで並列検索→重複排除→マージ** してから元クエリで re-rank する。従来はファイル名や周辺メタを指定しないとヒットしなかった情報も、直接質問するだけで拾えるようになった。

同じ仕組みは `/argus-brief` / `/argus-risk` / エンリッチメント（判断者・根拠の補完）にも適用されており、質問語彙と記載語彙が違うケースで検索精度が改善される。

---

## 6. `/argus-transcribe` — 会議録音の文字起こし・議事録生成
