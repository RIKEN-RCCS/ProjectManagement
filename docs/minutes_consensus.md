...
`generate_minutes_local.py` のオプション機能。同一プロンプトを N 回サンプリングし、
...
```
--consensus N                Self-consistency サンプリング数 (default: 1 = 単発)
...
```
/argus-transcribe Recording.mp4              # 単発（従来動作）
/argus-transcribe Recording.mp4 consensus=3  # Self-Consistency 有効
```
...
重要会議で手動 opt-in する運用を想定（デフォルトは単発）。
