...
python3 scripts/slack_pipeline.py -c <CHANNEL_ID> --skip-fetch --output overall.md

# 全体要約を生成してファイル保存 & Canvas 投稿
python3 scripts/slack_pipeline.py -c <CHANNEL_ID> --skip-fetch \
    --canvas-id <CANVAS_ID> --output overall.md
```
...
# 日付フィルタ付き
bash scripts/slack_to_pm.sh -c <CHANNEL_ID> --since 2026-01-01

# 確認用（DB保存なし）
bash scripts/slack_to_pm.sh -c <CHANNEL_ID> --dry-run
```
...
python3 scripts/pm_extractor.py -c <CHANNEL_ID> --list --since 2026-02-01
```
