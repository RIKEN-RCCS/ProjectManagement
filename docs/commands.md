...
| `--no-gate` | - | 記録前の3ゲート審査を行わない。既定では審査し `gate_verdict` 列に KEEP/DROP を残す（**件数ではなく内容で絞る**段。下記参照） |
| `--gate-backfill` | - | 既に記録済みの所見に3ゲート審査を後から当てる（未審査の行のみ。`--regate` で既判定も再審査）。審査基準を直したときに過去分へ当て直す |
| `--include-gate-dropped` | - | `--list-findings` で DROP と判定された所見も表示する。既定は KEEP と未審査のみ |
...
**3ゲート審査（内容での絞り込み）**: 読み手は 1 会議で 20〜30 件挙げてくるため、そのまま
台帳に入れると読まれない。記録前にマイルストーン基準の3ゲート（関連性 / 代替可能性 / 影響範囲）で
審査し、`gate_verdict` 列に KEEP / DROP と理由を残す（既定 ON）。**件数で切るのではなく内容で絞る** —
件数での打ち切りは本文がどこにも残らず「何を捨てたか」を検証できないため。

- **DROP でも行は消さない**。既定の表示から外すだけで、`--include-gate-dropped` /
  Console のトグルで見られる
- **未審査（NULL）は隠さない**。審査が動いていないことに気づけなくなるため。
  マイルストーン未登録時・審査失敗時は NULL のまま記録し WARN を出す
- 実測（2026-08-04、5会議 69 件）で KEEP 28 / DROP 41。**決定（方針・体制・期日・適用範囲）が残り、
  「〜を共有する」「〜を調整する」型の付随作業が落ちる**。期日付きの依頼も落ちるため、
  それはアクションアイテム側で管理する前提（PM 判断）
- **弱点**: 審査は主系統のLLM（`call_argus_llm`）が行う。つまり**主系統が落とした項目を
  主系統自身に「重要でない」と判定させている**。R8 の独立性を部分的に損なうため、
  行を残し DROP 件数を常に見えるようにしてある（DROP 比率が上がり続けるなら疑う）
