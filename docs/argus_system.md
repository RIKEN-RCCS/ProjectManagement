...
平日朝 8:57 に過去 30 日のデータを集計したブリーフィングを Canvas（`<CANVAS_ID>`）に投稿する。
...
朝のブリーフィングが「過去30日の全体像」なのに対し、こちらは「今日1日の出来事」に特化したサマリーを出す。
...
ログ確認: `tail -f logs/pm_argus_cron.log logs/pm_argus_daily.log`
...
CREATE TABLE chunks (
    ...
    source_db   TEXT NOT NULL,   -- 'minutes/Leader_Meeting.db' | '<CHANNEL_ID>.db' | 'docs_pm.db' | 'web_articles.db'
    record_id   TEXT,
...
現在の設定:
- 平日朝 8:57: `--brief-to-canvas` を自動実行（過去30日分ブリーフィング）
