...
### `/argus-knowledge` — 蒸留ナレッジの確認・編集

`data/knowledge.db` の閲覧・修正用 Slack コマンド。普段は使わない前提で、
`/argus-investigate` の回答末尾に表示された修正導線から呼び出すことを想定する。

```
/argus-knowledge list                       ← 一覧（最大30件）
/argus-knowledge KN-0042                    ← 詳細表示（rationale / sources / relations / audit）
/argus-knowledge invalidate KN-0042         ← deleted=1（論理削除）
/argus-knowledge restore KN-0042            ← deleted=0（復活）
/argus-knowledge supersede KN-0042 KN-0099  ← KN-0042.superseded_by = KN-0099
/argus-knowledge confidence KN-0042 medium  ← high / medium / low
```

すべての変更は `knowledge_audit` に `source='human_edit'`・`actor=<実行者の Slack 表示名>`
で記録される。CLI 版（`scripts/pm_knowledge_edit.py`）の `cmd_*` を直接呼ぶため挙動は同一。
詳細は `docs/distill_policy.md` 参照。
...
| `search_knowledge` | 蒸留ナレッジ knowledge.db を topic / current_state / tags / rationale で検索 |
| `get_knowledge` | 単一ナレッジ KN-XXXX を rationale / 代替案 / sources / relations までフル展開 |

**ナレッジ引用の自動検出**: 回答本文に `KN-XXXX` 形式が現れた場合、または
`search_knowledge` / `get_knowledge` が呼ばれた場合は、回答末尾に「## 引用したナレッジ」
セクションを自動付与し、各レコードの topic と修正導線（`/argus-knowledge invalidate` /
`/argus-knowledge supersede`）を表示する。`superseded_by` 立ちや `deleted=1` のレコードは
警告マーク付きで表示される。
...
#### 8. ナレッジ間の矛盾検知（`detect_knowledge_conflicts`）

`data/knowledge.db` の `knowledge_relations` に蒸留 / 人手で `conflicts_with` 行が
追加された際に通知する。

- 対象: `lookback_days`（デフォルト14日）以内に作成された conflicts_with 行
- 通知先: `leader_channel`（リーダー会議チャンネル）
- cooldown: 14日（同じ KN-XXXX↔KN-YYYY の組み合わせ単位。双方向は同一キー扱い）
- 1 回の巡回での最大通知数: `max_per_run`（デフォルト 5）
- `deleted=1` のレコードを含む組み合わせは除外
- 通知メッセージには `/argus-knowledge KN-XXXX`（詳細）/ `invalidate` / `supersede` の
  導線が含まれる
...
|---|---|---|
| `id` | INTEGER | PK |
| `event_type` | TEXT | 検出器名（`completion_confirm`・`overdue_reminder`・`deadline_warning`・`decision_followup`・`stale_alert`・`milestone_alert`・`weekly_trend_alert`・`knowledge_conflict`） |
...
patrol:
  ...
  knowledge_conflict:
    enabled: true                    # ナレッジ間矛盾検知の有効/無効
    cooldown_days: 14                # 同一矛盾組み合わせの再通知間隔
    lookback_days: 14                # この日数以内に作成された conflicts_with 行のみ通知
...
- `/argus-knowledge` — Short Description: 蒸留ナレッジ knowledge.db の閲覧・編集
