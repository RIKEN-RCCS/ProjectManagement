...
関連する記事を**キーワードマッチのみ**（LLM不使用）で検出し、リーダー会議チャンネルへ
「確認してください」と通知する。前提が今も妥当かの自動判定（confirm/contradict）は
行わない — 気付きの提供のみが目的。

- 対象: `state='active'` かつ `monitor_target` が非空の前提
- 記事の絞り込み: `lookback_days`（デフォルト14日）以内に `fetched_at` された記事
- マッチ判定: `monitor_target` を区切り文字（`, 、 ・ /` 空白）で分割した2文字以上の
  語が、記事の `title`/`summary`/`content` のいずれかに部分一致するか
- 通知先: `leader_channel`
