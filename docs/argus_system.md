...
```
ユーザー: /argus-brief 60 @富岳太郎（チャンネル: <channel_id>）
         ↓ Slack Socket Mode
...
│   ├── pm_qa_server.py      #   Slack Socket Mode デーモン（全 /argus-* を統合処理）
│   └── patrol/              #   Patrol サブモジュール
│       ├── state.py         #     冪等性DB（patrol_state.db）
│       ├── detect.py        #     検出ルール（LLM判定 + ルールベース）
│       ├── actions.py       #     アクション実行（Slack投稿・DB書き込み）
...
├── secretary_canvas_id.txt  # --brief-to-canvas の投稿先 Canvas ID（<CANVAS_ID>）
├── patrol_config.yaml   # Patrol Agent 設定（検出器の有効/無効・閾値・通知チャンネル）
├── patrol_state.db      # Patrol Agent 冪等性DB（自動作成、平文sqlite3）
├── slack.db             # Slack 統合DB（messages/replies を全チャンネル横断で保存。channel_id 列で絞り込み）
└── qa_index.db          # FTS5 統合インデックス（chunks + chunk_indexes junction で論理 index_name を分離）

logs/
├── pm_qa_server.log     # サーバーログ（全 /argus-* コマンド）
├── pm_qa_server.pid     # PIDファイル（起動中のみ存在）
├── pm_argus_daily.log   # cron 17時日次サマリー自動実行ログ
...
**背景知識の同梱（プロジェクト全体共通）**: `pm.db.decisions` のうち `rationale` が
入っている現役エントリ（`deleted=0`）を決定日降順で取り出し、
`fetch_background_knowledge()`（`pm_argus.py:284`）で Markdown 箇条書き化して
...
実行者がメンションされた投稿を別セクション「あなた宛のメンション」として生データ出力する。
...
**出力形式:**
1. LLM生成の日次サマリー（4観点）
2. 【あなた宛のメンション】セクション（生データ、タイムスタンプ・投稿者・本文）
   - 実行者の姓が本文に含まれる投稿を抽出
   - ゼロ件の場合はセクション自体を非表示

**用途:**
- 朝の日次確認（今日何が起きたか + 自分宛の依頼を見逃さない）
- Canvas の全体サマリーとは別に、個人向けにカスタマイズされた情報取得

**`/argus-brief` との違い:**
| 項目 | `/argus-brief` | `/argus-today` |
|---|---|---|
| 期間 | 直近30日（デフォルト） | 本日のみ |
| プロンプト | 優先アクション5件 | 4観点の活動サマリー |
| メンション | なし | あなた宛メンションを別セクション化 |
...
**処理フロー**:
1. ファイルをチャンネルから検索・ダウンロード（同名 VTT も自動検索: `{stem}.transcript.vtt` → `{stem}.vtt`、および解像度サフィックス `_3840x2160` 等を剥がしたフォールバックも試行）
2. mp4 等の動画の場合は `scripts/recording/slide_ocr.py` でスライドOCRを実行（`ffmpeg scene detect` でキーフレーム抽出 → マルチモーダルLLMで Markdown 化 → slide_context / terminology 生成）
3. Singularity コンテナ内で FFmpeg 変換（16kHz mono WAV） + Whisper large-v3 文字起こし（terminology がある場合は `--initial-prompt-extra` で固有名詞を initial_prompt に連結）
4. `generate_minutes_local.py` で LLM 議事録生成（マルチステージ: チャンク抽出 → 統合 → 決定事項・AI 抽出。VTT があれば `--vtt` 付きで話者情報を活用、スライドOCR結果があれば `--slide-context` で Stage 1/2/3 プロンプトに同梱）
5. 完成した議事録ファイルをスレッドにアップロード

**ファイル名の扱い**: Slack 上でファイル名を貼る際に太字記法（`*filename.mp4*`）やコード記法（`` `filename.mp4` ``）が付加されることがあるため、受け取ったテキストから両端の装飾文字（`*` / `_` / `` ` `` / `~` / 引用符・ゼロ幅文字）を自動的に除去してからファイル検索に使う。
...
### 夕方の日次サマリー（--today-only）

平日 17:00 に **本日のみ**のデータで日次サマリーを生成し Canvas（`<CANVAS_ID>`: リーダー会議Canvas）に投稿する。

```
0 17 * * 1-5  bash /lvs0/.../ProjectManagement/scripts/pm_argus_daily_summary.sh \
  >> logs/pm_argus_daily.log 2>&1
```

`pm_argus_daily_summary.sh` は以下を行う:
- `~/.secrets/slack_tokens.sh` / `~/.secrets/rivault_tokens.sh` を自動 source
- `pm_argus.py --brief-to-canvas --today-only --canvas-id <CANVAS_ID>` を実行

`--today-only` は `--days` / `--since` を無視し、**今日のデータのみ**を収集するフラグ。プロンプトの期間表示も「本日のデータ」と変わる。詳細: `docs/daily_summary_setup.md`。

ログ確認: `tail -f logs/pm_argus_daily.log`
...
### 検出ルール詳細（`argus/patrol/detect.py`）
...
- 通知先: `argus_config.yaml` の `patrol.leader_channel`
- cooldown: 7日
- 最大10件表示
...
#### 8. 外部シグナル検出（`detect_external_signals`）— Argus 垂直軸 機能1

`ledger_assumptions`（前提・意思決定台帳の「前提」エントリ）のうち `monitor_target`
（監視対象）が設定されているものについて、`data/web_articles.db` の外部Web記事から
関連する記事を**キーワードマッチ**で絞り込んだ後、**LLM で confirms（裏付け）/
contradicts（否定）/ neutral（キーワード一致のみで実質無関係）を判定**する
（設計書§5「共通の着地処理」の実装）。`neutral` は通知を抑制し（設計書§5
「通知過多の抑制」）、`confirms`/`contradicts` のみ通知・状態更新を行う。
`use_llm: false` または LLM 呼び出し失敗時は自動判定なしで通知のみのフォールバック
動作になる（旧仕様と同じ、人への気付き提供のみ）。

**着地処理の3作用**（設計書§5、`ledger_edges` の `monitors`/`depends_on` 辺を利用）:
1. **前提の確信度更新**: `confirms` → `last_reviewed_at` を更新（`confidence` が
   未設定なら `medium` に設定）。`contradicts` → `confidence='low'`,
   `state='review'` に変更し、**次回以降の自動監視対象から外れる**
...
- 通知先: `leader_channel`
- cooldown: 14日（同一 前提×記事 の組み合わせ単位。`target_key = "assumption:{id}:article:{id}"`。
  `neutral` 判定はcooldownを消費しない＝通知抑制のみで次回また同じ記事が再評価されうる）
- 1 回の巡回での最大通知数: `max_per_run`（デフォルト 5）
- `data/web_articles.db` は他DBと異なり**平文 sqlite3**（暗号化なし）。
  `db_utils.open_db_plain()` で開く

投入経路（前提の登録・`monitor_target` の設定）は `scripts/ingest/ledger.py`
`upsert_assumptions()` / `pm_ingest.py ledger`。前提の詳細は `docs/schema.md`
「Argus 垂直軸: 前提・意思決定台帳」参照。
...
```
argus/patrol/detect.py
  └─ 完了シグナル検出
       ↓
argus/patrol/actions.py: send_completion_confirm()
...
|---|---|---|
| `id` | INTEGER | PK |
| `event_type` | TEXT | 検出器名（`completion_confirm`・`overdue_reminder`・`deadline_warning`・`decision_followup`・`stale_alert`・`milestone_alert`・`weekly_trend_alert`・`external_signal`） |
...
patrol:
  ...
  leader_channel: <channel_id>        # エスカレーション・サマリー投稿先チャンネル
  ...
  external_signal:
    enabled: true                    # 外部シグナル検出の有効/無効（Argus 垂直軸 機能1）
    use_llm: true                    # LLM で confirms/contradicts/neutral を判定（neutralは通知抑制）
    cooldown_days: 14                # 同一 前提×記事 組み合わせの再通知間隔
    lookback_days: 14                # この日数以内に取得された記事のみ対象
    max_per_run: 5                   # 1回の巡回での最大通知数
```
...
```bash
# 通常実行
python3 scripts/argus/pm_argus_patrol.py

# DB・Slack 変更なし（動作確認用）
python3 scripts/argus/pm_argus_patrol.py --dry-run

# 特定の検出器のみ実行（--only に渡せる名前は下表参照）
python3 scripts/argus/pm_argus_patrol.py --only overdue
python3 scripts/argus/pm_argus_patrol.py --only completion,deadline

# 承認待ち一覧
python3 scripts/argus/pm_argus_patrol.py --list-pending
```
...
*/30 * * * 1-5 cd /lvs0/.../ProjectManagement && \
  ...
  ~/.venv_aarch64/bin/python3 scripts/argus/pm_argus_patrol.py \
...
~/.venv_aarch64/bin/python3 scripts/argus/pm_argus.py --brief-to-canvas

# リスク分析 → 標準出力のみ（Canvas 投稿なし）
~/.venv_aarch64/bin/python3 scripts/argus/pm_argus.py --risk --dry-run

# リスク分析 → Canvas に投稿
~/.venv_aarch64/bin/python3 scripts/argus/pm_argus.py --risk
...
### 統合インデックスと論理 index_name

2026-05-18 に `qa_pm.db` / `qa_pm-hpc.db` / `qa_pm-pmo.db` / `qa_pm-all.db` への DB 分割を廃止し、
**単一の `data/qa_index.db`** にチャンクを集約する構成に変更した。論理的な「インデックス」は
`chunk_indexes(chunk_id, index_name)` の junction テーブルで表現し、検索時に `index_name` で
絞り込む。同じチャンクが複数の論理 index に属する場合（例: `pm` と `pm-all`）は junction の
行を増やすだけで済み、本体テーブルは重複しない。

| 論理インデックス | 想定ソース |
|---|---|
| `pm` | Leader_Meeting + リーダー会議チャンネル |
| `pm-hpc` | Block1/2、SubWGx、BenchmarkWG_Meeting + HPCアプリ・ベンチマークWGチャンネル |
| `pm-pmo` | Co-design_Review等 + PMO系チャンネル |
| `pm-all` | 抽出時のナレッジ参照用にすべての議事録・チャンネルを横断する論理 index |

実際のマッピングは `data/argus_config.yaml` で定義する。`indices.{name}.db` の値は
互換のため残してよいが、`pm_qa_server.py` は値を無視して常に `data/qa_index.db` を使う。
...
indices:
  pm:
    # db: フィールドは互換のため残してよいが、pm_qa_server.py / pm_argus_agent.py は
    # 値を無視して常に data/qa_index.db を使う（chunk_indexes.index_name でフィルタ）。
  ...
  pm-all:
    # 抽出時の retrieve_knowledge_for_extraction() がデフォルトで参照する横断 index。
    minutes: [Leader_Meeting, Block1_Meeting, Block2_Meeting, SubWG3_Meeting, ...]
    channels: [<channel_id>, ...]
...
### 統合インデックスDB スキーマ（data/qa_index.db）
...
CREATE INDEX idx_chunks_source_record ON chunks(source_db, record_id);

-- 論理 index_name と chunk の M:N 関係。検索時はこの junction で絞り込む。
CREATE TABLE chunk_indexes (
    chunk_id   INTEGER NOT NULL,
    index_name TEXT NOT NULL,
    PRIMARY KEY (chunk_id, index_name),
    FOREIGN KEY (chunk_id) REFERENCES chunks(id) ON DELETE CASCADE
);
CREATE INDEX idx_chunk_indexes_index_name ON chunk_indexes(index_name);
...
-- (source_db, index_name) ごとの差分更新タイムスタンプ
...
検索系クエリ (`fts` / `fts_tokens` / LIKE / 最新フォールバック) はすべて
`JOIN chunk_indexes ci ON ci.chunk_id = c.id WHERE ci.index_name = ?` を付与して
論理 index で絞り込む（`pm_qa_server.py: _fts5_search / _fts_tokens_search / retrieve_chunks`）。
...
```
/argus-investigate 実行 (channel_id=<channel_id>)
  ↓
channel_map.get("<channel_id>") → なければ default_index="pm"
  ↓
index_name = "pm"
  ↓
data/qa_index.db に対して FTS5検索（chunk_indexes.index_name = 'pm' で絞り込み）
```

### 出典のチャンネル名表示

Slack要約の出典はチャンネルIDではなく人が読みやすい名称で表示する。
マッピングは `data/argus_config.yaml` の `channel_names:` から動的に解決する
（実値はチャンネル機密のためソースに持たない）。`pm_qa_server.py` / `pm_api.py` は
共通ローダ `web_utils.load_filter_presets()` 経由で参照する。

### ドキュメントレジストリ・本文の索引化

`pm_embed.py` は以下のソースを `data/qa_index.db` の `chunks` に投入し、
それぞれを `chunk_indexes(chunk_id, index_name)` 経由で論理 index に紐付ける。
...
pm_box_crawl.py       → box_docs.db         (Box フォルダ走査による本文 Markdown)
pm_box_relevance.py   → box_docs.db.box_files.relevance (LLM judging: core/related/noise)
pm_embed.py           → qa_index.db         (chunk_indexes で論理 index に紐付け)
/argus-investigate    → qa_index.db を index_name フィルタ付きで FTS5 検索
```
...
`--brief-to-canvas` CLI 実行時の投稿先 Canvas ID を1行で記載。
現在: `<CANVAS_ID>`
...
| 項目 | 現状 | 備考 |
|---|---|---|
| 検索方式 | SudachiPy形態素解析 + FTS5 + bge-m3 embedding のハイブリッド（RRF統合） | `scripts/argus/retrieval.py` |
| インデックス更新 | 手動または既存スクリプトへの追記 | crontabで定期自動更新が望ましい |
| 出典リンク | Slack投稿済み議事録のみリンクあり | `pm_minutes_import.py --post-to-slack` で増やせる |

### P1: セマンティック検索の導入（実現済み）

`scripts/argus/retrieval.py` が FTS5 検索とベクトル検索を RRF（Reciprocal Rank Fusion）で
統合するハイブリッド検索を実装済み（`retrieve_chunks_hybrid()` → `mcp_tools.py` 経由で
`/argus-investigate` から利用）。

- `qa_index.db.chunk_embeddings` に 1024 次元の bge-m3 ベクトルを保存（`pm_embed.py` が構築）
- `retrieve_chunks_vector()` が cosine similarity 検索、`_rrf_merge()` が FTS5 結果とマージ
- 鮮度スコアリング（`_recency_score()`、指数減衰）も RRF 後のスコアに加味
- `expand_query_hyde()` で質問を HyDE 風に拡張してから検索（P5 参照、実現済み）
- bge-m3 は RiVault が `bge-m3:567m` として提供しており、ローカル GPU 不要

旧 `data/knowledge.db`（蒸留ナレッジレイヤ）の Stage 2 で使っていた同種の類似度判定は
2026-06-16 の同DB全廃に伴い消滅したが、embedding 基盤自体は `qa_index.db` 側の
ハイブリッド検索として存続・発展している（`embed_utils.py` を共通利用）。
...
`retrieval.py::expand_query_hyde()` が質問を LLM で別表現（日英）に展開し、
複数クエリで検索してマージする HyDE（Hypothetical Document Embeddings）方式を実装済み。
`extract_search_keywords()` によるメタ語除去とあわせて P1 のハイブリッド検索の前段で使われる。
