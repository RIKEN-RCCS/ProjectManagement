...
```bash
python3 scripts/pm_box_crawl.py --debug-convert /path/to/file.docx
```
...
**修正ファイル**: `scripts/pm_box_crawl.py`
...
2. **手動テスト**:
   ...
   ```bash
   python3 scripts/pm_box_crawl.py --debug-convert /path/to/new.docx
   ```
...
```bash
# 修正版での全体変換
python3 scripts/pm_box_crawl.py --convert --workers 1 --force

# 特定ファイルのデバッグ
python3 scripts/pm_box_crawl.py --debug-convert /tmp/F_20250909_Minute_Project_regular_meeting_en.docx
...
- **変換パイプライン**: `pm_box_crawl.py` → `box_docs.db` (SQLCipher暗号化) → `pm_embed.py` → FTS5インデックス
