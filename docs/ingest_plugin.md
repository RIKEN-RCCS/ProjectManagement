...
│    │
│    ├─ PLUGINS 辞書にプラグインを登録
│    │    "slack"   → SlackIngestPlugin   (ingest/slack.py)
│    │    "minutes" → MinutesIngestPlugin (ingest/minutes.py)
│    │    "goals"   → GoalsIngestPlugin   (ingest/goals.py)
│    │
│    └─ IngestContext を構築して plugin.run() に渡す
│
├── ingest_plugin.py   ← IngestContext / IngestPlugin の定義
├── slack.py           ← Slack DB → pm.db プラグイン
...
`scripts/ingest/{source_name}.py` に作成する。
...
```python
# scripts/ingest/pm_ingest.py の PLUGINS 辞書に1行追加する
...
```sh
# 統合ランナー経由（推奨）
python3 scripts/ingest/pm_ingest.py my-api --my-api-endpoint https://...
python3 scripts/ingest/pm_ingest.py my-api --dry-run
python3 scripts/ingest/pm_ingest.py my-api --since 2026-01-01 --db data/pm.db
...
|---|---|---|---|---|
| `slack` | `SlackIngestPlugin` | `ingest/slack.py` | `decisions`, `action_items` | `slack_extractions`（thread_ts + channel_id） |
| `minutes` | `MinutesIngestPlugin` | `ingest/minutes.py` | `meetings`, `decisions`, `action_items` | `meetings.held_at + kind`（`--force` で上書き） |
| `goals` | `GoalsIngestPlugin` | `ingest/goals.py` | `goals`, `milestones` | `INSERT OR REPLACE`（完全同期） |
