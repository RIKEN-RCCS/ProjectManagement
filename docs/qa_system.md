...
```
ユーザー: /argus-ask 設計方針について（チャンネル: <CHANNEL_ID>）
         ↓ Slack Socket Mode
...
```
/argus-ask 実行 (channel_id=<CHANNEL_ID>)
  ↓
...
### ハイブリッド検索（Intent分類 + 構造化クエリ）

`/argus-ask` は質問の種類に応じてFTS5テキスト検索とpm.db構造化SQLクエリを使い分ける。

```
質問 → classify_intent（LLM Intent分類）
  ↓
intent=structured: run_structured_query → generate_answer(structured_context)
intent=text:       retrieve_chunks → rerank → generate_answer(chunks)
intent=hybrid:     両方実行 → generate_answer(structured_context + chunks)
```

**Intent分類カテゴリ**:
| カテゴリ | 説明 | 例 |
|---|---|---|
| `structured` | 担当者・期限・件数など構造化データで回答可能 | 「西澤さんの担当タスクは？」 |
| `text` | 議事録・議論の経緯などテキスト検索が必要 | 「設計方針について」 |
| `hybrid` | 両方が必要 | 「GPU性能に関する決定事項は？」 |

**構造化クエリのquery_type**:
| query_type | 実行内容 |
|---|---|
| `tasks` | action_items を assignee/status/milestone/keyword で検索 |
| `decisions` | decisions を keyword で検索 |
| `milestones` | マイルストーン進捗（`fetch_milestone_progress()`） |
| `overdue` | 期限超過アイテム一覧（`fetch_overdue_items()`） |
| `stats` | 統計サマリー（`fetch_summary_stats()`） |

**フォールバック**: Intent分類失敗 → `text`（既存FTSパス）。構造化クエリ結果0件 → FTS検索にフォールバック。

**レスポンスフッター**: `_（検索対象: pm / ハイブリッド検索）_` のように検索モードを表示。
...
/argus-ask            → qa_pm.db を検索（ドキュメントも検索対象）
```
...
/argus-ask を実行
  ...
  └─ executor.submit(_run_qa) ← バックグラウンドスレッドへ
         ├─ classify_intent() ← LLM Intent分類（1〜3秒）
         ├─ run_structured_query() ← pm.db SQLクエリ（数十ms、intent=structured/hybrid時）
         ├─ retrieve_chunks() ← FTS5検索（~数ms、intent=text/hybrid時）
...
- **Slash Command**: `/argus-ask`（Request URLは任意のHTTPS URL、Socket Modeでは無視される）
...
各チャンネルで `/argus-ask` を実行すると、そのチャンネルに対応したインデックスを検索する:

```
（<CHANNEL_ID> リーダー会議チャンネルで）
/argus-ask 設計方針に関する最近の議論は？
/argus-ask Benchparkハッカソンの内容を教えて

（<CHANNEL_ID> ベンチマークWGチャンネルで）
/argus-ask FFBのベンチマーク手法について
/argus-ask 直近の会議で決まった測定方針は？
```
