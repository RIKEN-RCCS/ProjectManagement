...
### 段階3: 蒸留時の二段階ゲート（実装済み 2026-05-18）

`pm_box_distill.py` は蒸留した候補をそのまま登録せず、**embedding 類似度 + LLM Stage 2 審査** の二段ゲートを通す:

1. **Stage 1**: ローカル LLM (gemma4) が `_DISTILL_POLICY` に従って候補を抽出
2. **Stage 2**: 各候補を `bge-m3:567m`（RiVault `/v1/embeddings`）で埋め込み、既存ナレッジと類似度比較
   - **類似度 ≥ 0.92 (`--merge-threshold`)** → 自動 merge（既存 KN-XXXX の `knowledge_sources` に当該ソースを追加するだけ。新規 KN は発番しない）
   - **類似度 ≥ 0.85 (`--review-threshold`)** → LLM (Kimi) に「これは KN-XXXX と同じ意味か?」を問う
   - **類似度 < 0.85** → LLM Stage 2 で keep / drop / merge_with の自由判定
3. LLM Stage 2 が判定に失敗した場合は **保守的に drop**（後で `pm_knowledge_edit.py --restore` で戻せる）

drop 判定された候補は `distill_state.status='quality_dropped'` に記録、`note` に理由を残す。
...
CREATE TABLE knowledge_embeddings (
    ...
    model         TEXT NOT NULL,           -- "bge-m3:567m"
...
```sh
# 通常蒸留（Stage 2 がデフォルト ON）
python3 scripts/pm_box_distill.py

# Stage 2 を無効化（段階1のみ。デバッグ用）
python3 scripts/pm_box_distill.py --no-two-stage

# 既存レコードに埋め込みを後追い計算（初回 / Stage 2 を有効化する前）
python3 scripts/pm_box_distill.py --embed-backfill

# 既存レコードに対して LLM 品質審査（後追いパージ）
python3 scripts/pm_box_distill.py --quality-only --quality-plan plan.csv
# プラン CSV を確認後
...
```sh
EMBED_API_BASE=http://llm.ai.r-ccs.riken.jp:11434/v1   # 省略時は RIVAULT_URL を流用
EMBED_API_KEY=$RIVAULT_TOKEN                            # 省略時は RIVAULT_TOKEN
EMBED_MODEL=bge-m3:567m                                 # 省略時のデフォルト
```

#### --quality-only の運用フロー

```sh
# 1. プラン CSV を生成（DB変更なし、所要時間: 1件あたり LLM 1回）
python3 scripts/pm_box_distill.py --quality-only --quality-plan data/quality_plan.csv

# 2. CSV を Excel で開いて確認（特に drop 候補と merge_target が妥当か）
...
CSV には `id, kind, confidence, verdict, merge_target, reason, topic, current_state` が含まれる。`verdict` は `keep` / `drop` / `merge_with` のいずれか。`apply` を指定しなければ DB は変更されない。
