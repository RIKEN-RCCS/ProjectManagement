...
**方針**: 流出面（層1〜3・監査・供給網固定）は実装済み。改竄側（R5・R13 — 引用スパン照合・
二段抽出・挙動指紋）は **PM 判断で当面着手しない**。
**R8（出自の独立した第2系統）は 2026-08-04 に保留** — 実装は残っているが**動かしていない**。
使えるモデルが無いためで、**この間 R8 対策は存在しない**（下記 0）。
...
- ~~R8 第2系統トリアージ~~ → **完了から取り下げ（2026-08-04）。下記「残っている作業」へ移動。**
...
> **2026-08-03 に既定 ON へ移行済み**。**R8 は 2026-08-04 に保留（対策なし）** — 上記 0 参照。
> K3 は「読み手（欠落検出）」限定で解禁済み（2026-08-03）。生成用途・視覚入力は保留のまま。

**残っている作業**（優先順）:
0. **R8 第2系統は保留中（2026-08-04、PM 判断）— この間 R8 対策は存在しない。**
   Scout（Llama-4-Scout-17B）は役割に耐えなかった（応答形式を守らない／ASR 誤認識を
   復元できない／記録された 10 件すべてが3ゲート審査で DROP）。**GLM-5.2 / Kimi-K3 に
   相当する能力で、かつ出自が主系統の外にあるモデルが現時点で無い。** 能力の足りない
   モデルを置いて「対策済み」にせず、無いと認めて保留する。
   保留の宣言は `config/sensitive_terms.yaml` の `second_opinion.on_hold`（第2系統を
   呼ぶ 3 経路すべてが WARN を出して飛ばす。`pm_selfcheck` の `second_opinion_on_hold`
   が毎回報告し、保留が忘れられるのを防ぐ）。
   解除条件: ① 出自が主系統の外で GLM-5.2 / K3 相当の読解力を持つモデルが配信される
...
7. **改竄側（R5・R8 の拡張・R13）— PM 判断で当面着手しない**。引用スパン照合・
   二段抽出・挙動指紋は保留する（R8 の第2系統そのものも上記 0 のとおり保留中で、
   **「実装済み」ではない**）
