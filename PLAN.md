...
### argus-investigate 特定ファイル(Box資料)スコープ検索

**ステータス**: 着手（2026-07-17）。承認済み計画は `~/.claude/plans/c-fluffy-lemur.md`。

**背景**: 検索は index_name / since でしか絞れず、embedding 済みの特定 Box 資料「1本だけ」に
QA を仕掛けられない。retrieval は各チャンクの record_id を SELECT するのに WHERE で未使用。

**方針**: retrieval 層に `record_ids` フィルタ（`c.record_id IN (…)`）を汎用追加し、
ファイル名 → `box_docs.db.box_files` → box_file_id 解決を通す。効かせ方は決定論的ピン
（`--file` フラグ → AgentContext.record_ids に固定、全 search を強制スコープ）＋ LLM ツール引数
（search_text/hybrid の `file`）の両方。対象は Box 資料のみ（source_type='box_document'）。
決定論ピン時の search_entity スコープ漏れは配線 or ツール除外で塞ぐ。

**残作業**: 実装（argus-implementer 委譲中）→ argus-code-reviewer レビュー →
qa デーモン再起動（argus-ops）→ 実 Slack 検証。
