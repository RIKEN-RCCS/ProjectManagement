...
**ステータス**: コード修正完了 (2026-06-05)、`~/.secrets/rivault_tokens.sh` 編集とデーモン再起動が残作業。

**手順**:
1. ユーザーが `~/.secrets/rivault_tokens.sh` を以下に編集:
   ```sh
   export RIVAULT_MODEL="deepseek-ai/DeepSeek-V4-Flash"
   export ARGUS_PREFER_RIVAULT=1
   ```
2. `bash scripts/pm_daemon.sh stop qa && bash scripts/pm_daemon.sh start qa`
3. Slack で `/argus-investigate` を 1 件試して動作確認
