...
### 4. Slack 抽出のナレッジ検索の第一段が機械的キーワード抽出（残 1 件）

**ステータス**: 保留中。旧「argus-investigate と同種バグの追加調査」の残り 1 件
（他 2 件は 2026-07-24 に決着 — LOG.md 参照）。**2026-07-24 に実装を精査し記述を実態へ訂正**:
「スレッド全文を投入」は誤りで、実際は `retrieve_knowledge_for_extraction`
（`scripts/utils/cli_utils.py:320`）が SudachiPy 名詞抽出→先頭 15 語で検索し、下流は
investigate と同じ HyDE 拡張 + LLM rerank を共有している。

- 残る差分は第一段のみ: investigate は LLM rewrite（メタ語除去・固有名詞正規化）、
  Slack 抽出は「出現順の先頭 15 名詞」。長いスレッドでは冒頭の雑談名詞がスロットを食い、
  複数話題が 1 クエリに混線する
- LLM 化の移植は容易だがプロンプトは別物（質問の蒸留ではなくスレッドの主題エンティティ抽出）。
  effectは precision 改善（ノイズ低減）に限られ、深い語彙ミスマッチ（保留構想 5）の根治ではない
  （investigate 側の案A失敗で実証済み）。ingest はスレッド毎に走るためスレッド毎 +1 LLM 呼のコスト増
- 検証は recall_eval ハーネス流用不可（investigate 質問用）。サンプルスレッド N 件で
  取得ナレッジの関連性を判定する軽量 A/B が必要
