...
### Box報告書の図・グラフ言語化 — 残: 新規core文書への自動付与

**ステータス**: 全core一括ロールアウト完了（2026-07-18、経緯は LOG.md）。残るは自動化のみ。

**残作業**: 新規 core 文書への図OCR自動付与（2パス化: 軽量変換→relevance判定→core を
図OCR付き再変換→embed の夜間自動化）。現状、新規文書は手動 `--figures --force` が必要。
既存core文書のBox側更新は source_modified_at 検知＋figures維持で自動追従済み。
外れ値 xlsx由来PDF 1件（~2.05M字）は意図的に未処理（図価値低、skipped.tsv 記録）。
