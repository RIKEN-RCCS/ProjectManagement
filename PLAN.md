...
図60ブロック、FTS/ベクトル両検索でヒット確認）。ハードニング2件（OCR応答 None ガード、
pm_embed の陳腐化チャンク record 単位掃除 `purge_stale_record_chunks`）もコミット済み。
...
- pm_box_update.sh の2パス化（変換→relevance判定→core図OCR再変換→embed）の自動化検討。
