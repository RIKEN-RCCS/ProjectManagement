...
### investigate に read_document ツール追加 ＋ map段の4並列化

**ステータス**: 実装中（2026-07-20 着手）。詳細計画は `~/.claude/plans/c-fluffy-lemur.md`。

全文読込QA（map-reduce）の品質を通常 investigate にも波及させる。全検索の map-reduce 置換は
コーパス規模（core+related 903文書≒740窓 → 直列1.2〜3時間/質問）で棄却し、ツール型ハイブリッドを採用:
(1) エージェントに `read_document(file, question)` ツールを追加（検索で見つけた文書を LLM 判断で全文読込）、
(2) map 段を ThreadPool(4) 並列化（7窓 34〜61秒 → 目安15〜25秒）。
`AgentContext.deadline` で残り時間予算をツールに伝搬（上限180s/呼）。
