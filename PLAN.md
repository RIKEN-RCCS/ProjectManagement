...
### Box報告書の図・グラフ言語化（マルチモーダルOCR）

**ステータス**: 実装・単一PDFパイロット・ハードニング完了（2026-07-17）。**全core展開はユーザー指示で保留中**。

**背景**: `_convert_pdf` は pdftotext が100字超だとマルチモーダルOCRを行わないため、
テキストが取れる報告書のグラフ・図・ダイアグラムが索引に一切載らず、argus-investigate が
視覚情報を認識できない。基盤（`_pdf_to_images`/`_ocr_image`/`get_ocr_endpoints`/
`_convert_via_multimodal`）は既存。

**方針（承認済み）**: テキスト維持＋図OCR追加 / core資料のみ。
- pdftotext 本文は保持し、各ページ画像に「図・グラフ言語化」専用OCRパス（新プロンプト
  `FIGURE_VERBALIZE_PROMPT`：図種別・軸・系列・数値・傾向・示唆を記述、テキスト段落は無視）を
  追加し、ページ本文と図説明を `\f`（pdftotext のページ境界）で対応付けてマージ。
- 対象は relevance='core' のみ。2パス運用（軽量変換→relevance判定→core を `--force` で
  図OCR付き再変換→embed）。CLI フラグ `--figures` で発火、内部で relevance='core' を AND。
- モデルは既存 `LOCAL_OCR_MODEL`/`RIVAULT_OCR_MODEL`（`get_ocr_endpoints`）。未設定なら警告してスキップ。
- 再embeddingは content_md 変化で自動（chunk本文一致が崩れ→backfill）。全面ロールアウト
  （全core一括 `--force --figures`）はコスト大のため実装・単体検証とは分離しユーザー起動。

**完了済み**: 実装・レビュー・修正、図読み取りの数値詳細化（`FIGURE_VERBALIZE_MAX_TOKENS=8192`）、
単一PDF（box_file_id=2146797875532）でのパイロット2回（詳細版で char 90,952→139,224、
図60ブロック、FTS/ベクトル両検索でヒット確認）。ハードニング: OCR応答 None ガード、
pm_embed の陳腐化チャンク掃除 `purge_stale_record_chunks`、Fable レビュー起点の
ページ対応マージ修正・OCR失敗の partial マーキング・Box更新検知（`source_modified_at`
等値比較）・figures 自動維持（cron でも剥がれない、エンドポイント不可時はスキップ→リトライ）。

**残作業（保留中・ユーザー起動待ち）**:
- 全core一括ロールアウト（`--convert --figures --force`、約258件/8,100ページ相当、概算2〜3時間）。
  外れ値 xlsx由来PDF（box_file_id 精査で~1,850ページ、図価値低）は除外候補。ファイル並列は1〜2。
  ※このロールアウトが既存行の source_modified_at 基準も確立する（それまで既存行の
  Box更新は検知されない＝雪崩防止とのトレードオフ）。
- ロールアウト後の再embed（増分実行で自動、陳腐化掃除も新コードで自動）。
- 新規 core 文書への図OCR自動付与（2パス化の残り。現状は新規は手動 --figures --force）。
