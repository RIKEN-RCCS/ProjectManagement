...
### read_document のツール選択率向上（保留・様子見）

**ステータス**: 保留（2026-07-20）。read_document ツール本体＋map 4並列化は実装・マージ済み
（経緯は LOG.md 2026-07-20）。

e2e 検証で `read_document` が glm-5.2 の0件ツール呼び出しにより発火せず、「情報なし」誤答を
1件観測。実装は `--file` 経由で「呼ばれれば動く」ことを裏付け済みだが、通常経路での発火が
モデル依存。再開時の選択肢: (1) 初期自動検索の上位が単一 Box 文書に集中したら read_document を
自動エスカレーション（計画で当初スコープ外とした案）、(2) forced-synthesis 前に read_document を
促すナッジ、(3) ツール説明文・システムプロンプトの強化。現状の確実な代替は `--file` 直指定。
