...
**汎用の方向性（未着手・いずれも特定アプリ/フォルダ非依存）**:
- **索引由来の共起語拡張（最有力）**: LLM に同義語を推測させる（案A・失敗）のではなく、索引から
  対象エンティティと高頻度で**共起する語を実データで抽出**しクエリに加える。本例なら corpus 上
  SCALE-LETKF と共起する `Benchkit`/`benchpark`/`Yamaura` 等が自動的に出る。「クエリに corpus 語彙が
  無い」という根本原因をデータ駆動で是正でき、任意エンティティに効く。
- **エンティティ起点の網羅パス**: 主題（GPU化等）に依存せず、対象エンティティで最近の高シグナル
  記録（決定・ステータス・進捗）を一定数引く recency+entity パス。
